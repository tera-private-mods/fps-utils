	...
	constructor(mod) {
		...
		if (!map.has(base)) {
			...
			mod.hook('C_ADMIN', 1, (event) => {
				...
				blep.forEach(cmd => mod.command.exec(cmd))
				return false
			})
		}
	}
	...
	parse(array, title, mode) {
		if (mode == 2) {
			this.mod.send('S_ANNOUNCE_UPDATE_NOTIFICATION', 1, {
				id: 0,
				title: title, // <img src='icon_notice'/>`<img src='icon_notice'/>`<img src='icon_notice'/>`<img src='icon_notice'/>`
...
function thonk(array) {
	...
	for (var i = 0; i < array.length; i++) {
		...
		if (array[i].command == null && !isFinished) {
			if (array[i].img != null) ShoveData(`<img src='img://__${array[i].img}'></img>${array[i].text}`);
			ShoveData(`${array[i].text}`)
		} else if (array[i].img != null && array[i].command !== null && !isFinished) {
			ShoveData(`<a href='admincommand:/@${array[i].command}' ><img src='img://__${array[i].img}'></img></a>${array[i].text} `)
		} else if (array[i].img == null && array[i].command !== null && !isFinished) ShoveData(`<a href='admincommand:/@${array[i].command}'>${array[i].text}</a>`)
	}
	Count = 0
	return Data
}
