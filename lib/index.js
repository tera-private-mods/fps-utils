...
module.exports = function FpsUtils(mod) {
		...
		switch (page) {
			...
			case "npcMain":
				...
				TmpData.push(
					...
					{ text: `<br><br><font color="${PinkClr}" size="+16">If you want to search for npc with space between it's name, you've to add the whole name inside quotations, e.g. \`fps gui npcsearch "Bay Kamara"\`</font>` }
				)
			...
			default:
				gui.parse([
					...
					{ text: `&#09;&#09;&#09;` },
					{ text: `<font color="${mod.settings.HideMonsterDeathAnimation ? GreenClr : RedClr}" size="+17">[NPCs Death Ani]</font>`, command: `fps monsterdeathani;fps gui` },
					{ text: "&nbsp;&nbsp;" },
					{ text: `<font color="${mod.settings.ActionScripts ? GreenClr : RedClr}" size="+17">[NPCs Zoom-in]</font>`, command: `fps npczoom;fps gui` },
					{ text: "&nbsp;&nbsp;" },
					...
					{ text: "&nbsp;&nbsp;" },
					{ text: `<font color="${mod.settings.OnlyParty ? GreenClr : RedClr}" size="+17">[Only Party]</font>`, command: `fps party;fps gui` },
					{ text: `<br><br><br><br>` },
	...
	function JsonSearch(nameKey, array, arg, title) {
		for (let i = 0; i < array.length; i++) {
			if (array[i].Name.startsWith(nameKey) && arg == `starts`) TmpData.push({ command: `fps npc hide ${array[i].HuntingZoneId} ${array[i].TemplateId};fps gui npc ${nameKey}`, text: `<font color="${mod.settings.NpcsBlacklist.some(arrVal => array[i].HuntingZoneId == arrVal.zone && array[i].TemplateId == arrVal.templateId) ? GreenClr : RedClr}" size="+17">${array[i].Name} - ${array[i].HuntingZoneId}, ${array[i].TemplateId}</font><br>` })
			if (array[i].Name.includes(nameKey) && arg == `search`) TmpData.push({ command: `fps npc hide ${array[i].HuntingZoneId} ${array[i].TemplateId};fps gui npcsearch ${nameKey}`, text: `<font color="${mod.settings.NpcsBlacklist.some(arrVal => array[i].HuntingZoneId == arrVal.zone && array[i].TemplateId == arrVal.templateId) ? GreenClr : RedClr}" size="+17">${array[i].Name} - ${array[i].HuntingZoneId}, ${array[i].TemplateId}</font><br>` })
			if (!isNaN(arg) && array[i].nameKey == arg) return true;
		}
	...
	function sUnicastTransformData(event) {
		if (event.gameId == 0 || EqGid(event.gameId)) return;
		if (mod.settings.ShowStyle) return false;
		if (mod.settings.Hideguildlogos) {
			event.guildLogo = '';
			return true;
	...
	function sAbnormalityRefresh(event) {
		if (HiddenUsers[event.target]) return false;
		if ((mod.settings.HideBlacklistedAbnormalities || mod.settings.HideAllAbnormalities) && (SpawnedUsers[[10306, 10316, 12001, 12003, 12120, 12130].includes(event.target)] || SpawnedUsers[[10306, 10316, 12001, 12003, 12120, 12130].includes(event.source)])) return false;
	}

	function sAbnormalityFail(event) {
		if ([3000021, 99020000].includes(event.id)) return false;
	}
	...
	function sStartUserProjectile(event) {
		if (mod.settings.ProjDebug) {
			...
			if (SpawnedUsers[event.gameId]) log('Projectile', 'Started', 'from', SpawnedUsers[event.source].name, event.skill.id)
		}
		...
		switch (key) {
			...
			case "hide":
				if (typeof arg === "string" && arg !== null) {
					if (mod.settings.PlayersBlacklist.includes(arg)) return Msg(`Player "${arg}"` + ` Already hidden`.clr(RedClr) + '.');
					else
						if ((mod.settings.ClassesNames.includes(arg) && !mod.settings.ClassesBlacklist.includes(arg)) || (mod.settings.RolesNames.includes(arg) && !mod.settings.RolesBlacklist.includes(arg))) {
							...
							return;
						} else if (mod.settings.ClassesBlacklist.includes(arg) || mod.settings.RolesBlacklist.includes(arg)) return Msg(`Class/Role "${arg}"` + ` Already hidden`.clr(RedClr) + '.');
			...
			case "abnormalities":
				switch (arg) {
						...
						if (!arg2 || !arg3) {
							...
							if (!mod.settings.AbnormalitiesBlacklist.includes(arg3)) {
								if (arg2 == 'add') {
									...
									return;
								} else if (arg2 == 'remv') return Msg(`Blacklisted Abnormalities` + ` can't remove "${arg3}" as it's not there`.clr(GrayClr) + '.');

							} else if (mod.settings.AbnormalitiesBlacklist.includes(arg3)) {
								if (arg2 == 'add') return Msg(`Blacklisted Abnormalities` + ` can't add "${arg3}" as it's already there`.clr(GrayClr) + '.');
								else if (arg2 == 'remv') {
					...
					case "debug":
						...
						mod.settings.AbnDebug ? Msg(`Abnormalities debug ${"started".clr(GreenClr)}, check your proxy console for details.`) : Msg(`Abnormalities debug ${"stopped".clr(RedClr)}.`);
						break;
					default:
			...
			case "projectile":
				switch (arg) {
						...
						if (!arg2 || !arg3) {
							...
							if (!mod.settings.ProjectilesBlacklist.includes(arg3)) {
								if (arg2 == 'add') {
									...
									return;
								} else if (arg2 == 'remv') return Msg(`Blacklisted projectile` + ` can't remove "${arg3}" as it's not there`.clr(GrayClr) + '.');
							} else if (mod.settings.ProjectilesBlacklist.includes(arg3)) {
								if (arg2 == 'add') return Msg(`Blacklisted projectile` + ` can't add "${arg3}" as it's already there`.clr(GrayClr) + '.');
								else if (arg2 == 'remv') {
					...
					case "debug":
						...
						mod.settings.ProjDebug ? Msg(`Projectile debug ${"started".clr(GreenClr)}, check your proxy console for details.`) : Msg(`Projectile debug ${"stopped".clr(RedClr)}.`);
						break;
					default:
