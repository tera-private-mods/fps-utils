...
module.exports = function FpsUtils(mod) {
	...
	function sSpawnDropItem(event) {
		...
		if (mod.settings.HideBlacklistedDrop) {
			for (const p in event.owners) if ((event.owners[p].playerId.toString() === MyPlayerId.toString()) || event.owners.length === 0) return;
	...
	mod.hook('S_LOGIN', mod.majorPatchVersion >= 81 ? 13 : 12, sLogin)
