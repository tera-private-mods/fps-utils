...
module.exports = function FpsUtils(mod) {
	...
	const NotCP = typeof global.TeraProxy === 'undefined' || typeof global.TeraProxy.UILanguage === 'undefined';
	...
	const InvisibleNpcs = { 8: [6011], 12: [1], 23: [201, 202], 34: [8000], 61: [9901], 62: [9001, 9002, 9003, 9004, 9901], 83: [9901], 207: [9901], 223: [9901], 230: [9901] };
		...
		switch (page) {
			...
			case "help":
				gui.parse([
					...
					{ text: `<font color="${HBlC}">mpnumbers</font>                <font color="${DBlC}">N/A</font>                    <font color="${HPkC}">!fps mpnumbers</font></font><br>` },
					{ text: `<font color="${HBlC}">pet      </font>                <font color="${DBlC}">N/A</font>                    <font color="${HPkC}">!fps raid</font></font><br>` },
					{ text: `<font color="${HBlC}">guardian </font>                <font color="${DBlC}">N/A</font>                    <font color="${HPkC}">!fps guardian</font></font><br>` },
	...
	function ShowSpecificPlayer(name) {
		for (let i in HUsers) {
			if (HUsers[i].name.toString().toLowerCase() === name.toLowerCase()) {
				...
				mod.toClient('S_SPAWN_USER', NotCP ? 14 : 15, HUsers[i]);
				delete HUsers[i];
			}
		}
	}
		...
		switch (type) {
			...
			case 'pet':
				switch (whose) {
					case 'own':
						for (let p in SPets) {
							if (!EqGid(SPets[p].ownerId)) continue
							mod.toClient('S_REQUEST_DESPAWN_SERVANT', 1, { gameId: SPets[p].gameId, despawnType: 1 })
							HPets[SPets[p].gameId] = SPets[p]
						}
						break;
					case 'others':
						for (let p in SPets) {
		...
		switch (type) {
			...
			case 'pet':
				switch (whose) {
					case 'own':
						for (let p in HPets) {
							if (!EqGid(HPets[p].ownerId)) continue
							mod.toClient('S_REQUEST_SPAWN_SERVANT', NotCP ? 1 : 2, HPets[p])
							delete HPets[p]
						}
						break;
					case 'others':
	...
	function ModifySkillResult(event) {
		...
		if (mod.settings.Hit_Damage && (EqGid(event.source) || EqGid(event.owner) || UserNPCs.includes(event.templateId) && event.skill.huntingZoneId === 1023)) {
	...
	function sLogin(event) {
		...
		AbnDebug = false;
		AllowedAchUps = 2;
		if (mod.settings.StreamMode) console.log("\x1b[94mINFO\x1b[34m [FPS-UTILS]\x1b[39m - Steam Mode is enabled, No messages will be sent in-game messages until it's disabled.");
	}
	...
	function sLeaveParty() {
		...
		if (mod.settings.RaidAutoChange) {
			if (LastState !== null || mod.settings.Mode !== 2) return;
	...
	function sPartyMemberList(event) {
		...
		if (mod.settings.RaidAutoChange) {
			if (event.members.length >= 18) {
				if (mod.settings.Mode === 3 || (LastState !== null && mod.settings.Mode === 2)) return;
	...
	function sAbnormalityBegin(event) {
		...
		if ([90442103, 90442107].includes(event.id)) { event.id = 47702000; return true; }
		...
		if (mod.settings.HideBlacklistedAbnormalities && (mod.settings.AbnormalitiesBlacklist.includes(event.id) || BlockedAbns.includes(event.id))) return false;
		if (mod.settings.HideAllAbnormalities && (SUsers[event.target] || SUsers[event.source] || mod.settings.AbnormalitiesBlacklist.includes(event.id) || BlockedAbns.includes(event.id))) return false;
	}
	...
	function sActionStage(event) {
		...
		if ((mod.settings.Mode >= 2) || HUsers[event.gameId] || mod.settings.ClassesData[ClassID(event.templateId)].CD_HideBlacklistedSkills || (mod.settings.HideBlacklistedSkills && mod.settings.ClassesData[ClassID(event.templateId)].CD_SkillsBlacklist.includes(Math.floor(event.skill.id / 10000).toString()))) {
			UpdateUserLoc(event);
			return false;
		}
	}
	...
	function sStartUserProjectile(event) {
		...
		if (mod.settings.Mode !== 3 && event.skill.id === 90120) return;
		if (mod.settings.PvpTraps && TrapsIds.includes(event.skill.id)) return;
	...
	function sSpawnDropItem(event) {
		...
		if (mod.settings.HideBlacklistedDrop && mod.settings.DropBlacklist.includes(event.item)) return false;
	...
	function sFieldEventOnLeave() {
		if (mod.settings.GuardianAutoChange) {
			if (LastGState !== null || mod.settings.Mode !== 2) return;
			mod.command.exec(`fps mode ${LastGState}`);
			LastGState = null;
		}
	}
	...
	mod.hook('S_SPAWN_USER', NotCP ? 14 : 15, Last_Hook, sSpawnUser)
	mod.hook('S_SPAWN_USER', NotCP ? 14 : 15, Last_Hookfn, sSpawnUserfn)
	...
	mod.hook('S_UNICAST_TRANSFORM_DATA', NotCP ? 5 : 6, Last_Hook, sUnicastTransformData)
	...
	mod.hook('S_REQUEST_SPAWN_SERVANT', NotCP ? 1 : 2, sRequestSpawnServant)
	...
	mod.hook('S_EACH_SKILL_RESULT', 13, Last_Hook, sEachSkillResult)
	mod.hook('S_SPAWN_DROPITEM', NotCP ? 7 : 8, sSpawnDropItem)
	...
	mod.command.add(['fps', '!fps', 'fps-utils', '!fps-utils'], (key, arg, arg2, arg3) => {
		switch (key) {
			...
			case "pet": case "pets": case "servant": case "servants":
				switch (arg) {
					...
					default:
						...
						Msg(`Others summoned NPCs ${mod.settings.HideOthersServants ? `<font color="${GrnC}">Hidden</font>` : `<font color="${RedC}">Shown</font>`}.`);
						break;
				}
				break;
			case "hpnumbers":
			...
			case "stream":
				...
				Msg(`Stream mode is ${mod.settings.StreamMode ? `<font color="${GrnC}">Enabled</font>` : `<font color="${RedC}">Disabled</font>`}.`); // It will only show when disabled.
				if (mod.settings.StreamMode) console.log("\x1b[94mINFO\x1b[34m [FPS-UTILS]\x1b[39m - Steam Mode has been enabled, No messages will be sent in-game messages until it's disabled.");
