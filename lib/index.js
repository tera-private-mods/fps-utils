...
module.exports = function FpsUtils(mod) {
	...
	function ProjectileBCheck(event) {
		...
		if ((mod.settings.Mode !== 3 && event.skill.id === 90120) || (mod.settings.PvpTraps && TrapsIds.includes(event.skill.id))) return undefined;
				...
				if (typeof arg === "string" && arg !== null) {
					if (mod.settings.PlayersBlacklist.includes(arg)) return Msg(`Player '${arg}' <font color="${RedC}">Already hidden</font>.`);
						...
						if ((GameJobs.includes(arg) && !mod.settings.ClassesBlacklist.includes(arg)) || (GameRoles.includes(arg) && !mod.settings.RolesBlacklist.includes(arg))) {
							for (const cD in mod.settings.ClassesData) {
								if ((mod.settings.ClassesData[cD].name === arg || mod.settings.ClassesData[cD].role.includes(arg)) && cD.isHidden !== true) {
									mod.settings.ClassesData[cD].isHidden = true;
									if (mod.settings.ClassesData[cD].name === arg) mod.settings.ClassesBlacklist.push(arg);
									if (mod.settings.ClassesData[cD].role.includes(arg)) mod.settings.RolesBlacklist.push(arg);
									let classtohide = mod.settings.ClassesData[cD].model;
									for (const sUser in SUsers) if (ClassID(SUsers[sUser].templateId) === classtohide) HideSpecificPlayerByName(SUsers[sUser].name);
								}
							}
			...
			case "show":
				if (typeof arg === "string" && arg !== null) {
					...
					if ((GameJobs.includes(arg) && mod.settings.ClassesBlacklist.includes(arg)) || (mod.settings.RolesBlacklist.includes(arg) && GameRoles.includes(arg))) {
						for (const cD in mod.settings.ClassesData) {
							if (mod.settings.ClassesData[cD].name === arg || mod.settings.ClassesData[cD].role.includes(arg)) {
								if (mod.settings.ClassesData[cD].name === arg) RemoveEntity(mod.settings.ClassesBlacklist, arg);
								if (mod.settings.ClassesData[cD].role.includes(arg)) RemoveEntity(mod.settings.RolesBlacklist, arg);
								mod.settings.ClassesData[cD].isHidden = false;
								let classToShow = mod.settings.ClassesData[cD].model;
			...
			case "party":
				...
				if (mod.settings.OnlyParty) {
					for (const sUser in SUsers) {
						...
						else if (HUsers.includes(SUsers[sUser].name)) ShowSpecificPlayerByName(SUsers[sUser].name)
					}
				} else ShowAllPlayers();
				...
				switch (arg) {
					...
					case "class":
						if (GameJobs.includes(arg2)) {
							for (const cD in mod.settings.ClassesData) {
								if (mod.settings.ClassesData[cD].name === arg2) {
									if (arg3 && !isNaN(arg3) && arg3 < 50) {
										if (mod.settings.ClassesData[cD].CD_SkillsBlacklist.includes(arg3)) {
											let index = mod.settings.ClassesData[cD].CD_SkillsBlacklist.indexOf(arg3);
											if (index !== -1) {
												mod.settings.ClassesData[cD].CD_SkillsBlacklist.splice(index, 1);
												Msg(`Skill ID '${arg3}' <font color="${RedC}">Shown</font> for class '${arg2}'.`);
											}
											return;
										} else {
											mod.settings.ClassesData[cD].CD_SkillsBlacklist.push(arg3);
											Msg(`Skill ID '${arg3}' <font color="${GrnC}">Hidden</font> for class '${arg2}'.`);
											return;
										}
									} else {
										mod.settings.ClassesData[cD].CD_HideBlacklistedSkills = !mod.settings.ClassesData[cD].CD_HideBlacklistedSkills;
										Msg(`Skills for '${arg2}' class ${mod.settings.ClassesData[cD].CD_HideBlacklistedSkills ? `<font color="${GrnC}">Hidden</font>` : `<font color="${RedC}">Shown</font>`}.`);
										return;
									}
								}
							}
