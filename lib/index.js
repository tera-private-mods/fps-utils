...
module.exports = function FpsUtils(mod) {
	...
	const NpcData = require("./data/npcData.json"),
		...
		BnzC = '#BC8F8F', SlvC = '#9A9A9A', GldC = '#DAA520', PnkC = '#FFB7C5', HPkC = '#FF69B4', CPKC = '#ED5D92', RedC = '#FE6F5E', GrnC = '#4DE19C', LPrC = '#E0B0FF',
		PrpC = '#9966CC', LBlC = '#4DDCE1', BluC = '#436EEE', DBlC = '#00575e', HBlC = '#08B3E5', GryC = '#778899', YlwC = '#C0B94D', OngC = '#FFBE5E';

	/*const FishingNpcs = {
		8: [6011], //Vexatus ~Frog
		12: [1, 2], //Cutthroat ~Crab & Bay kamara
		23: [201, 202], // Frog & Bee
		34: [8000], //Muricai ~Goat
		...
		212: [1034], //Cutthroat ~Reg
		223: [9901, 9906, 9907, 9915, 9925], //Poporia - Lake of Tears
		230: [9901], //Veritas District - Seren's Lake
		362: [1201, 1202, 1301, 1302] //Cutthroat ~Skl
	};*/

	const FishingNpcs = { // TODO; Delete this
		8: [6011], //frog
	...
	const PvPProjs = [
		30120, 30220, 30320, //Shock trap ----Sorcerer
		90120, 90220, 90320, 90420, 90520, 90620, 90720, 90820, 90920, //Slow trap ----Archer
		100120, 100220, 100320, 100420, 100520, //Stun trap ----Archer
		120220, 120320, 120420, 120520, 120620, 120720, 120820, 120920, //Void ----Sorcerer
		150120, 150220, 150320, 150420, 150520, 150620, 150720, //Explosion trap ----Archer
		170120, 170220, 170320, 170420, 170520, 170620, 170720, 170820, 170920, //Pain-mark trap ----Sorcerer
		230120, 230220, 230320, //Stun-range trap ----Archer
		250120, 250220, 250320, 250420, 250520, 250620, 250720, 250820, 250920, 251020 //Explosion-range trap ----Archer
	];

	const UserSumNPCs = [
		12345, //Tree DJ ----Ninja
		1100100, 1100101, //Shadow SA ----Warrior
		1023000, 1023001, 1023002, 1023003, 1023004, 1023005, 1023006, 1023007, 1023008, 1023009, 1023010, 1023011, 1023012, 1023013, 1023014, //ToP ----Mystic
		1023200, 1023201, 1023202, 1023203, 1023204, 1023205, 1023206, 1023207, 1023208, 1023209, 1023210, 1023211, //ToL ----Mystic
		1023300, 1023301, 1023302, 1023303, 1023304, 1023305, 1023306, 1023307, 1023308, 1023309, 1023310, 1023311, 1023312, //ToV ----Mystic
		1023400, 1023401, 1023402, 1023403, 1023404, 1023405, //ToW ----Mystic
		1024000, 1024001, //Warding Totem ----Mystic
		1024100, //Numbing Totem ----Mystic
		1024200, //Speed Totem ----Mystic
		1024300, //Assailing Totem ----Mystic
		2100100, 2100101, //Shadow SF ----Warrior
		10235001, 10235002, 10235003, 10235004, 10235005, 10235006, 10235007, 10235008, 10235009, 10235010, 10235011, 10235012, 10235013, 10235014, 10235015, 10235016, 10235017, 10235100, //ToP II ----Mystic
		10236001, 10236002, 10236003, 10236004, 10236005, 10236006, 10236007, 10236008, 10236009, 10236010, 10236011, 10236012, 10236013, 10236014, 10236015, 10236100, //ToL II ----Mystic
		10237001, 10237002, 10237003, 10237004, 10237005, 10237006, 10237007, 10237008, 10237009, 10237010, 10237011, 10237012, 10237013, 10237014, 10237015, 10237100, //ToV II ----Mystic
		10238001, 10238002, 10238003, 10238004, 10238005, 10238006, 10238007, 10238008, 10238100, //ToW II ----Mystic
		10239003, //Blob ----Mystic
		30301001, 30301002, 30301003, 30301004, 30301005, //ST ----Gunner
		30302001, 30302002, 30302003, 30302004, 30302005, 30302006, 30302007, //HB ----Gunner
		30303001, 30303002, 30303003, 30303004, 30303005, 30303006, 30303007, //HB-Keen ----Gunner
	];
	...
	function HideSpecificNpc(hz, ti) {
		console.log(`HideSpecificNpc ${hz} "${typeof hz}"- ${ti} "${typeof ti}"`)
		for (let i in SNpcs) {
	...
	function ShowSpecificNpc(hz, ti) {
		console.log(`ShowSpecificNpc ${hz} "${typeof hz}"- ${ti} "${typeof ti}"`)
		for (let i in HNpcs) {
	...
	function NpcBCheck(event) { // TODO; ₛₛₛₛₛₛₕₕₕₕₕₕₕₕ
		let blocked = false;
	...
	function sNpcLocation(event) {
		...
		if (HNpcs[event.gameId]) {
			...
			return false; // TODO; make sure the destory is done properly so npcs don't freeze in their spot after spawn.
		}
	}
