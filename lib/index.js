...
module.exports = function FpsUtils(mod) {
	...
	function GuiHandler(page, arg) {
		switch (page) {
			...
			case "show":
				...
				for (let i in SUsers) {
					TmpData.push({ text: `<font color="${mod.settings.PlayersBlacklist.indexOf(SUsers[i].name.toLowerCase()) !== -1 ? GrnC : RedC}" size="+17">${SUsers[i].name}</font><br>`, command: mod.settings.PlayersBlacklist.indexOf(SUsers[i].name.toLowerCase()) !== -1 ? `fps show ${SUsers[i].name};fps gui show` : `fps hide  ${SUsers[i].name};fps gui show` })
				}
				...
				switch (whose) {
					case 'own':
						for (let p in HPets) {
							...
							mod.toClient('S_REQUEST_SPAWN_SERVANT', 3, HPets[p])
							delete HPets[p]
						}
						break;
					case 'others':
	...
	function sSpawnUser(event) {
		...
		if (mod.settings.Mode === 3 || mod.settings.PlayersBlacklist.indexOf(event.name.toLowerCase()) !== -1 || mod.settings.ClassesData[ClassID(event.templateId)].isHidden || (mod.settings.OnlyParty && !PMembers.includes(event.name))) {
	...
	if (mod.majorPatchVersion < 86) mod.hook('S_UPDATE_ACHIEVEMENT_PROGRESS', 1, Last_Hookfn, sUpdateAchievementProgress) //Probably will enable this next patch, when the fix comes; Soon.™
	...
	mod.hook('S_REQUEST_SPAWN_SERVANT', 3, Last_Hook, sRequestSpawnServant)
	...
	mod.hook('S_ABNORMALITY_END', 1, Last_Hook, sAbnormalityEnd)
	...
	mod.command.add(['fps', '!fps', 'fps-utils', '!fps-utils'], (key, arg, arg2, arg3) => {
		...
		switch (key) {
						...
						if ((GameJobs.includes(arg) && !mod.settings.ClassesBlacklist.includes(arg)) || (GameRoles.includes(arg) && !mod.settings.RolesBlacklist.includes(arg))) {
							for (let i in mod.settings.ClassesData) {
								if ((mod.settings.ClassesData[i].name === arg || mod.settings.ClassesData[i].role.includes(arg)) && mod.settings.ClassesData[i].isHidden !== true) {
									...
									for (let i in SUsers) { if (ClassID(SUsers[i].templateId) === classtohide) HideSpecificPlayerByName(SUsers[i].name); }
								}
							}
					...
					if ((GameJobs.includes(arg) && mod.settings.ClassesBlacklist.includes(arg)) || (mod.settings.RolesBlacklist.includes(arg) && GameRoles.includes(arg))) {
						for (let i in mod.settings.ClassesData) {
							if (mod.settings.ClassesData[i].name === arg || mod.settings.ClassesData[i].role.includes(arg)) {
								...
								for (let i in HUsers) {
									if (ClassID(HUsers[i].templateId) === classToShow) ShowSpecificPlayerByName(HUsers[i].name);
								}
							}
						}
			...
			case "pvptraps":
				...
				break;
			case "list":
				Msg(`<font color="${PnkC}">Hidden players: ${mod.settings.PlayersBlacklist.length ? mod.settings.PlayersBlacklist : 0}</font>.`);
				Msg(`<font color="${PnkC}">Hidden classes: ${mod.settings.ClassesBlacklist.length ? mod.settings.ClassesBlacklist : 0}</font>.`);
				Msg(`<font color="${PnkC}">Hidden roles: ${mod.settings.RolesBlacklist.length ? mod.settings.RolesBlacklist : 0}</font>.`);
				break;
