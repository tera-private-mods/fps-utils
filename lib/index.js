...
module.exports = function FpsUtils(mod) {
	...
	const UserBCheck = user => { for (const player of mod.settings.PlayersBlacklist) if (player.toLowerCase() === user.toLowerCase()) return true }
	...
	function sLeaveParty() {
		PMembers = []
		//if (mod.settings.OnlyParty) mod.command.exec('fps party')
	...
	function sAbnormalityBegin(event) {
		...
		if ([90442103, 90442107].indexOf(event.id) !== -1) { event.id = 47702000; return true; }
		if (ImportantAbns.indexOf(event.id) !== -1) return;
	...
	if ([86, 87].indexOf(mod.majorPatchVersion) === -1) mod.hook('S_UPDATE_ACHIEVEMENT_PROGRESS', 1, Last_Hookfn, sUpdateAchievementProgress) // Temp check for patchVersion coz Unsure if all regions got 88 yet. (SEA/TH?)
			...
			case "quicklink":
				switch (arg) {
					...
					case "reset":
						...
						break;
					case "searchengine":
						mod.toClient('S_SHOW_AWESOMIUMWEB_SHOP', 1, { link: 'https://www.dogpile.com/' });	// To-do... delete this feature; most search engines have/going to remove support for TLSv1.0 & 1.1 which tera uses Starting on January 1st, 2020.
						break;
					default:
