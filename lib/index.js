...
module.exports = function FpsUtils(mod) {
	...
	if (notcaaliproxy) {
		const settingsVersion = require('../module').options.settingsVersion
		mod.settings = require('./setting_migration')(mod.settings._version > settingsVersion ? undefined : mod.settings._version, settingsVersion, mod.settings)
	}
	...
	function GuiHandler(page, arg) {
		switch (page) {
			...
			case "proj":
				...
				TmpData = []
				break;
			case "help": //TODO; Better formating. Attempt #3 (Noto Sans JP-KR-SC-TC) #wtb html support.
				gui.parse([
					{ text: `<font color="${PurpleClr}" size="+24"><p align="right">MAIN MENU</p></font><br>`, command: `fps gui` },
					{ text: `<font color="${PinkClr}" size="+17"><p align="right">This page is under development. If you have messed up font check the ReadMe for list of commands, Thank you.</p></font><br>` },
					{ text: `<font size="20">` },
	...
	function sLeaveParty() {
		...
		if (mod.settings.RaidAutoChange) {
			if (mod.settings.Mode !== 2 || LastState === null) return;
	...
	function sPartyMemberList(event) {
		...
		if (mod.settings.RaidAutoChange) {
			if (event.members.length >= 18) {
				if (mod.settings.Mode === 3 || (mod.settings.Mode === 2 && LastState !== null)) return;
		...
		switch (key) {
			...
			case "screenabns":
				if (arg == 'hide') {
					...
					arg2 = Number(arg2);
					let found = mod.settings.OwnAbnormalsBlacklist.some(m => { return m === arg2; });
					if (found) {
						Msg(`Abnormal id '${arg2}'` + " Removed from the blacklist".clr(RedClr) + '.')
						mod.settings.OwnAbnormalsBlacklist = mod.settings.OwnAbnormalsBlacklist.filter(obj => { return obj != arg2; })
					} else {
