...
module.exports = function FpsUtils(mod) {
	...
	const BnzC = '#BC8F8F', SlvC = '#9A9A9A', GldC = '#DAA520', PnkC = '#FFB7C5', HPkC = '#FF69B4', CPKC = '#ED5D92', RedC = '#FE6F5E', GrnC = '#4DE19C', LPrC = '#E0B0FF', PrpC = '#9966CC', LBlC = '#4DDCE1', BluC = '#436EEE', DBlC = '#5BC0BE', HBlC = '#08B3E5', GryC = '#778899', YlwC = '#C0B94D', OngC = '#FFBE5E';
	const InvisibleNpcs = { 8: [6011], 12: [1, 2], 23: [201, 202], 34: [8000], 61: [9901], 62: [1017, 1035, 1039, 1040, 1041, 1042, 1900, 1901, 1902, 1903, 9001, 9002, 9003, 9004, 9901], 83: [9901], 207: [9901], 212: [1034], 223: [9901, 9906, 9907, 9915, 9925], 230: [9901], 362: [1201, 1202, 1301, 1302] };
	...
	const SpamAchievementIds = [51, 377, 391, 466, 467, 468, 469, 470, 471, 472, 473, 476, 611, 1408, 1744, 1745, 1769, 1770, 1771, 1942, 1943, 1952, 1992, 2032, 2033, 2036, 2037, 2038, 2057, 2062, 2071, 2078, 2081, 2082, 2083, 2084, 2088, 2089, 2095, 6005, 9817, 28723, 28724, 28725, 28726, 28727, 28728, 28729, 28730, 28731, 28732, 28733, 28734, 28735, 28736, 510005, 510012, 510019, 510026, 510027, 510033, 510034, 510035, 510036, 510037, 510038, 510039, 510040, 510041, 520029, 520030, 520032, 520033, 520034, 520035, 520048, 520049, 520050, 520051, 525000, 525001, 525002, 525003, 525004, 525005, 525006, 525007, 525008, 525009, 525010, 525011, 525012, 525013, 525014, 525015, 525016, 525017, 525018, 525019, 525020, 525021, 525022, 525023, 525024, 525025, 5010000];
	const ImportantAbns = [
		// Bahaar
		90440009,	// Hammer stun

		90442106,	// Endurance debuff at placement(hit) of meteor

		90442110,	// Circle before meteor #1 (Wrath bg0)
		90442111,	// Circle before meteor #2 (Wrath bg17)
		90442112,	// Circle before meteor #3 (Wrath bg79)
		90442113,	// Circle before meteor #4 (Wrath bg52)
		90442114,	// Circle before meteor #5 (Wrath bg12)

		90442400,	// 0     degree arrow & circle
		90442401,	// 45    degree arrow & circle
		90442402,	// 90    degree arrow & circle
		90442403,	// 135   degree arrow & circle
		90442404,	// 180   degree arrow & circle
		90442405,	// 225   degree arrow & circle
		90442406,	// 270   degree arrow & circle
		90442407,	// 315   degree arrow & circle
		90442408,	// 360   degree arrow & circle
		...
		switch (page) {
			...
			default:
				gui.parse([
					...
					{ text: `<font color="${YlwC}" size="+21">Self(own):</font>` }, { text: "&#09;" },
					...
					{ text: `<font color="${YlwC}" size="+21">NPCs:</font>` }, { text: "&#09;&#09;" },
					...
					{ text: `<font color="${YlwC}" size="+21">Skills:</font>` }, { text: "&#09;&#09;" },
					...
					{ text: `<font color="${YlwC}" size="+21">Abnormal:</font>` }, { text: "&#09;" },
					...
					{ text: `<font color="${YlwC}" size="+21">Projectile:</font>` }, { text: "&#09;" },
					...
					{ text: `<font color="${mod.settings.PvpTraps ? GrnC : RedC}" size="+17">[Show Traps]</font><br><br><br><br>`, command: "fps pvptraps;fps gui" },

					{ text: `<font color="${BluC}" size="+22">Quick Links:</font><br>` },
					{ text: `<font color="${YlwC}" size="+21">UI:</font>` }, { text: "&#09;&#09;" },
					...
					{ text: `<font color="${YlwC}" size="+21">Group:</font>` }, { text: "&#09;" },
					{ text: `<font color="${OngC}" size="+18">[Reset]</font>`, command: "fps quicklink reset" }, { text: "&nbsp;&nbsp;&nbsp;&nbsp;" },
					...
					{ text: `<font color="${YlwC}" size="+21">Extra:</font>` }, { text: "&#09;" },
					{ text: `<font color="${CPKC}" size="+18">[Lobby]</font>`, command: "fps quicklink lobby" }, { text: "&#09;&#09;&#09;&#09;&#09;&#09;&nbsp;" },
					{ text: `<font color="${HPkC}" size="+18">[!! Instant Exit]</font><br>`, command: "fps quicklink instantexit" }
	...
	function ClassNameFromID(name) {
		for (const Class_data of Object.keys(mod.settings.ClassesData)) {
	...
	function ModifySkillResult(event) {
		...
		if (mod.settings.Hit_Damage && (EqGid(event.source) || EqGid(event.owner) || UserNPCs.includes(event.templateId) && event.skill.huntingZoneId === 1023)) {
			event.id = event.damage = event.type = event.noctEffect = event.reaction.id = 0;
			event.crit = event.stackExplode = event.blocked = false;
	...
	function sUpdateAchievementProgress(event) {
		if (FirstAchUpdates) { FirstAchUpdates--; return; }
		for (const Achieve of event.achievements) if (SpamAchievementIds.indexOf(Achieve.id) > -1) return false;
	}
	...
	function sUserAppearanceChangefn(event) {
		if (EqGid(event.id)) return;
		if (mod.settings.ShowStyle) return false;
	}
	...
	function sUserExternalChangefn(event) {
		if (EqGid(event.gameId)) return;
		if (mod.settings.ShowStyle) return false;
	}
	...
	function sFearMoveStage(event) {
		if ((!EqGid(event.target) && mod.settings.Mode >= 1) || HUsers[event.target] || HNpcs[event.target]) return false;
	}
	...
	mod.hook('S_UPDATE_ACHIEVEMENT_PROGRESS', 1, { order: 99999, filter: { fake: null } }, sUpdateAchievementProgress)
	...
	mod.hook('S_USER_MOVETYPE', 'raw', { order: 110 }, sUserMoveType)	// I found that some public module were force reapplying this? so here's the order for that.
	mod.hook('S_PARTY_MEMBER_LIST', 7, sPartyMemberList)
	mod.hook('S_USER_APPEARANCE_CHANGE', 1, { order: 9999 }, sUserAppearanceChange)
	mod.hook('S_USER_APPEARANCE_CHANGE', 1, { order: 99999, filter: { fake: null } }, sUserAppearanceChangefn)
	mod.hook('S_USER_EXTERNAL_CHANGE', 7, { order: 9999 }, sUserExternalChange)
	mod.hook('S_USER_EXTERNAL_CHANGE', 7, { order: 99999, filter: { fake: null } }, sUserExternalChangefn)
	...
	mod.hook('S_EACH_SKILL_RESULT', 12, { order: 10010 }, sEachSkillResult)
