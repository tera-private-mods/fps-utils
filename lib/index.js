		...
		switch (page) {
			...
			case "show":
				...
				for (let i in SpawnedPlayers) {
					TmpData.push({ text: `<font color="${mod.settings.PlayersBlacklist.includes(SpawnedPlayers[i].name) ? GreenClr : RedClr}" size="+17">${SpawnedPlayers[i].name}</font><br>`, command: mod.settings.PlayersBlacklist.includes(SpawnedPlayers[i].name) ? `fps show ${SpawnedPlayers[i].name};fps gui show` : `fps hide  ${SpawnedPlayers[i].name};fps gui show` })
				}
