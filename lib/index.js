...
module.exports = function FpsUtils(mod) {
	...
	const BnzC = '#BC8F8F', SlvC = '#9A9A9A', GldC = '#DAA520', PnkC = '#FFB7C5', HPkC = '#FF69B4', CPKC = '#ED5D92', RedC = '#FE6F5E', GrnC = '#4DE19C', LPrC = '#E0B0FF', PrpC = '#9966CC', LBlC = '#4DDCE1', BluC = '#436EEE', DBlC = '#00575e', HBlC = '#08B3E5', GryC = '#778899', YlwC = '#C0B94D', OngC = '#FFBE5E';
		...
		switch (page) {
			...
			default:
				gui.parse([
					...
					{ text: `<font color="${mod.settings.HideOwnBlacklistedAbns ? GrnC : RedC}" size="+17">[Dizziness]</font>`, command: "fps screenabns;fps gui" }, { text: "&nbsp;&nbsp;" },
					{ text: `<font color="${mod.settings.HideHpMpNumbers ? GrnC : RedC}" size="+17">[HpMp Nums]</font><br>`, command: "fps hpmpnumbers;fps gui" }, { text: "&#09;&#09;&#09;" },
	...
	function ShowSpecificNpc(hz, ti) {
		for (let i in HNpcs) {
			if (HNpcs[i].huntingZoneId === hz && HNpcs[i].templateId === ti) {
				mod.toClient('S_SPAWN_NPC', mod.majorPatchVersion < 79 ? 10 : 11, HNpcs[i]);
				delete HNpcs[i];
			}
		}
	}
			...
			case 'own':
				for (let v in HNpcs) {
					if (EqGid(HNpcs[v].owner) && UserSumNPCs.includes(HNpcs[v].templateId) && HNpcs[v].huntingZoneId === 1023) {
						mod.toClient('S_SPAWN_NPC', mod.majorPatchVersion < 79 ? 10 : 11, HNpcs[v]);
						delete HNpcs[v];
					}
				}
				break;
	...
	function ModifySkillResult(event) {
		...
		if (mod.settings.Hit_Damage && (EqGid(event.source) || EqGid(event.owner) || UserSumNPCs.includes(event.templateId) && event.skill.huntingZoneId === 1023)) {
	...
	function sSystemMessage(event) {
		...
		if (msg.id === 'SMT_FIELD_EVENT_WORLD_ANNOUNCE') return false;			// Superior guardian spam.
		if (msg.id === 'SMT_KOREAN_RATING_TEENAGER_PROHIBITED') return false;	// You've been playing for X hours spam.
	}
	...
	function sPlayerChangeMp(event) {
		if (!mod.settings.HideHpMpNumbers || !EqGid(event.target)) return;
		if (event.type != 0) {
			event.type = 0;
			return true;
		}
	...
	//mod.hook('S_QUEST_VILLAGER_INFO', 'raw', sQuestVillagerInfo) Currently the opcode for this ain't public, but it will be soon™. Never™. and others went private... re. :\
	...
	mod.hook('S_USER_APPEARANCE_CHANGE', 1, { order: 99999, filter: { fake: null } }, sUserAppearanceChangefn) // fix for the other script which force re-apply the whole apperance.
	mod.hook('S_USER_EXTERNAL_CHANGE', 7, { order: 9999 }, sUserExternalChange)
	mod.hook('S_USER_EXTERNAL_CHANGE', 7, { order: 99999, filter: { fake: null } }, sUserExternalChangefn) // fix for the other script which force re-apply the whole apperance.
	mod.hook('S_UNICAST_TRANSFORM_DATA', 5, { order: 99999 }, sUnicastTransformData)

	mod.hook('S_SPAWN_NPC', mod.majorPatchVersion < 79 ? 10 : 11, sSpawnNpc)
	...
	mod.command.add(['fps', '!fps', 'fps-utils', '!fps-utils'], (key, arg, arg2, arg3) => {
		switch (key) {
			...
			case "fishing":
				...
				break;
			case "hpmpnumbers":
				mod.settings.HideHpMpNumbers = !mod.settings.HideHpMpNumbers;
				Msg(`Own HpMp numbers ${mod.settings.HideHpMpNumbers ? `<font color="${GrnC}">Hidden</font>` : `<font color="${RedC}">Shown</font>`}.`)
				break;
			case "help":
