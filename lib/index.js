...
module.exports = function FpsUtils(mod) {
	...
	function sFearMoveStage(event) {
		if (typeof event.target !== "undefined" && ((!EqGid(event.target) && mod.settings.Mode === 3) || HUsers[event.target] || HNpcs[event.target])) return false;
		if (typeof event.gameId !== "undefined" && ((!EqGid(event.gameId) && mod.settings.Mode === 3) || HUsers[event.gameId] || HNpcs[event.gameId])) return false;
	}
	...
	mod.hook('S_GUILD_NAME', 1, Last_Hook, sGuildName)
	mod.hook('S_APPLY_TITLE', 2, Last_Hook, sApplyTitle)
	...
	mod.hook('S_FEARMOVE_STAGE', NotCP ? 1 : 2, Last_Hook, sFearMoveStage)
	mod.hook('S_FEARMOVE_END', NotCP ? 1 : 2, Last_Hook, sFearMoveEnd)

	mod.hook('S_REQUEST_SPAWN_SERVANT', NotCP ? 1 : 2, Last_Hook, sRequestSpawnServant)
	...
	mod.hook('S_SPAWN_DROPITEM', NotCP ? 7 : 8, Last_Hook, sSpawnDropItem)
