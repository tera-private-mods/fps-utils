...
module.exports = function FpsUtils(mod) {
	...
	const FishingNpcs = {
		8: [6011], //frog
		12: [1, 2], //crab, bay kamara(not aggressive type)
		23: [201, 202], //frog, bee
		34: [8000], //goat
		61: [9805, 9901, 9902, 9903, 9904, 9905, 9906, 9907, 9908], //Ostgarath - Vexatus Swamp
		62: [1006, 1007, 1008, 1009, 1010, 1011, 1012, 1013, 1014, 1015, 1016, 1017, 1020, 1024, 1025, 1026, 1031, 1032, 1033, 1034, 1035, 1038, 1039, 1040, 1041, 1042, 1043, 1501, 1900, 1901, 1902, 1903, 8100, 9001, 9002, 9003, 9004, 9805, 9901, 9902, 9903, 9904, 9905], //Ostgarath - Cutthroat Harbor
		83: [9805, 9901, 9902, 9903, 9904, 9905], //Westonia - Celsian Lake
		99: [1180, 1181, 1182, 1183, 1184, 1185], //All invisible ?_? - unknown area.
		207: [9805, 9901, 9902, 9903, 9904, 9905], //Arcadia - Muricai
		212: [1034], //Cutthroat followup
		223: [9805, 9901, 9902, 9903, 9904, 9905, 9906, 9907, 9915, 9925], //Poporia - Lake of Tears
		230: [1041, 1042, 1043, 1044, 1045, 1046, 1084, 9805, 9901, 9902, 9903, 9904, 9905, 9906], //Veritas District - Seren's Lake
		362: [1201, 1202, 1301, 1302] //Cutthroat followup
	}
		...
		switch (page) {
			...
			case "help":
				gui.parse([
					...
					{ text: `<font color='${HBlueClr}'>    fishing    </font>             <font color='${DBlueClr}'>N/A</font>                         <font color='${HPinkClr}'>!fps fishing</font><br>` },
					{ text: `</font>` }
	...
	function Msg(msg) {
		if (notcaaliproxy) mod.command.message(`<font color="${LPurpleClr}">[FPS] ${msg}</font>`)
	...
	function sSystemMessage(event) {
		if (!mod.settings.Fishing) return;
		let SystemMessage = mod.parseSystemMessage(event.message)
	...
	function sSpawnNpc(event) {
		...
		if (event.huntingZoneId === 1023) {
			...
			if (!UsersSummonedNPCs.includes(event.id)) return;
	...
	//mod.hook('S_QUEST_VILLAGER_INFO', 'raw', sQuestVillagerInfo) opcode not public... Waiting for confirmation from caali/pinkie.
	...
	//mod.hook('S_USER_APPEARANCE_CHANGE', 1, { order: 9999 }, sUserAppearanceChange) missing opcode, pinkie.
		...
		switch (key) {
			...
			case "abnormalities":
				switch (arg) {
					...
					case "debug":
						...
						if (mod.settings.AbnDebug) Msg(`Abnormalities debug <font color="${GreenClr}">started</font>, check your proxy console for details.`)
				...
				break;
			case "guildlogo":
				if (ChangeOnCd) return Msg(`<font color="${PinkClr}">Try again in 3 seconds</font>.`);
			...
			case "projectile":
				switch (arg) {
					...
					case "debug":
						...
						if (mod.settings.ProjDebug) Msg(`Projectile debug <font color="${GreenClr}">started</font>, check your proxy console for details.`)
			...
			case "fishing":
				...
				if (mod.settings.Fishing) {
					HideFishingNpcs();
					if (!Alerted) {
						Msg(`<font color="${PinkClr}">Reminder: All fishing npcs won't spawn until you turn off fishing mode</font>.`);
						Alerted = true;
					}
