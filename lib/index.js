...
module.exports = function FpsUtils(mod) {
				...
				switch (whose) {
					case 'own':
						for (let p in HPets) {
							...
							mod.toClient('S_REQUEST_SPAWN_SERVANT', 4, HPets[p])
							delete HPets[p]
						}
						break;
					case 'others':
	...
	function sSpawnEventSeed(event) {
		if (HUsers[NotCP ? event.gameId : event.owner]) {
			HSeedBoxes[NotCP ? event.target : event.gameId] = event;
			return false;
		}
	}
	...
	function sResultEventSeed(event) {
		if (HSeedBoxes[NotCP ? event.cid : event.gameId]) return false;
	}
	...
	mod.hook('S_SPAWN_EVENT_SEED', 1, NotCP ? 1 : 2, Last_Hookfn, sSpawnEventSeed)
	mod.hook('S_UPDATE_EVENT_SEED_STATE', 3, Last_Hookfn, sUpdateEventSeedState)
	mod.hook('S_RESULT_EVENT_SEED', NotCP ? 1 : 2, Last_Hookfn, sResultEventSeed)
	...
	mod.command.add(['fps', '!fps', 'fps-utils', '!fps-utils'], (key, arg, arg2, arg3) => {
		...
		switch (key) {
			case "b":
				mod.command.exec('fps skills blacklist');
				break;
			case "g": case "gui":
				GuiHandler(arg, arg2);
				break;
