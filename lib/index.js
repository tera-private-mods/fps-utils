...
module.exports = function FpsUtils(mod) {
	...
	function sActionStage(event) {
		...
		if ((mod.settings.Mode >= 2) || HUsers[event.gameId] || mod.settings.ClassesData[ClassID(event.templateId)].CD_HideBlacklistedSkills) {
			UpdateUserLoc(event);
			return false;
		}
		if (Classic_NOTCP) {
			if (mod.settings.HideBlacklistedSkills && mod.settings.ClassesData[ClassID(event.templateId)].CD_SkillsBlacklist.includes(Math.floor((event.skill - 0x4000000) / 10000).toString())) {
				UpdateUserLoc(event);
				return false;
			}
		} else if (mod.settings.HideBlacklistedSkills && mod.settings.ClassesData[ClassID(event.templateId)].CD_SkillsBlacklist.includes(Math.floor(event.skill.id / 10000).toString())) {
			UpdateUserLoc(event);
			return false;
		}
	}
	...
	function sStartUserProjectile(event) {
		if (ProjDebug) {
			if (EqGid(event.gameId)) if (Classic_NOTCP) log('Projectile', 'Started', 'from', MyName, event.skill - 0x4000000); else log('Projectile', 'Started', 'from', MyName, event.skill.id);
			if (SUsers[event.gameId]) if (Classic_NOTCP) log('Projectile', 'Started', 'from', SUsers[event.gameId].name, event.skill - 0x4000000); else log('Projectile', 'Started', 'from', SUsers[event.gameId].name, event.skill.id);
		}
		if (mod.settings.PvpTraps) {
			if (Classic_NOTCP && TrapsIds.includes(event.skill - 0x4000000)) return;
			else if (TrapsIds.includes(event.skill.id)) return;
		}
		if ((mod.settings.HideBlacklistedProjectiles || mod.settings.HideAllProjectiles)) { if (Classic_NOTCP && mod.settings.ProjectilesBlacklist.includes(event.skill - 0x4000000)) return false; else if (mod.settings.ProjectilesBlacklist.includes(event.skill.id)) return false; }
	...
	function sSpawnProjectile(event) {
		...
		if (ProjDebug) {
			if (EqGid(event.gameId)) if (Classic_NOTCP) log('Projectile', 'Spawned', 'from', MyName, event.skill - 0x4000000); else log('Projectile', 'Spawned', 'from', MyName, event.skill.id);
			if (SUsers[event.gameId]) if (Classic_NOTCP) log('Projectile', 'Spawned', 'from', SUsers[event.gameId].name, event.skill - 0x4000000); else log('Projectile', 'Spawned', 'from', SUsers[event.gameId].name, event.skill.id);
		}
		if (mod.settings.PvpTraps) {
			if (Classic_NOTCP && TrapsIds.includes(event.skill - 0x4000000)) return;
			else if (TrapsIds.includes(event.skill.id)) return;
