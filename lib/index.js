...
module.exports = function FpsUtils(mod) {
	...
	function ShowSpecificPlayerByName(name) {
		for (const hUser in HUsers) {
			if (HUsers[hUser].name.toString().toLowerCase() === name.toLowerCase()) {
				...
				mod.toClient('S_SPAWN_USER', NotCP ? 15 : 16, HUsers[hUser]);
				delete HUsers[hUser];
				break;
			}
		}
	...
	mod.hook('S_SPAWN_USER', NotCP ? 15 : 16, Last_Hook, sSpawnUser)
	mod.hook('S_SPAWN_USER', NotCP ? 15 : 16, Last_Hookfn, sSpawnUserfn)
	...
	mod.hook('S_SPAWN_DROPITEM', NotCP ? 8 : 9, Last_Hook, sSpawnDropItem)
					...
					if (mod.settings.PlayersBlacklist.includes(arg)) return Msg(`Player '${arg}' <font color="${RedC}">Already hidden</font>.`);
						...
						if ((GameJobs.includes(arg) && !mod.settings.ClassesBlacklist.includes(arg)) || (GameRoles.includes(arg) && !mod.settings.RolesBlacklist.includes(arg))) {
							for (const cD in mod.settings.ClassesData) {
								if ((mod.settings.ClassesData[cD].name === arg || mod.settings.ClassesData[cD].role.includes(arg)) && !mod.settings.ClassesData[cD].isHidden) {
