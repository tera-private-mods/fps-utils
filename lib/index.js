...
module.exports = function FpsUtils(mod) {
	...
	function ShowSpecificPlayerByName(name) {
		for (const hUser in HUsers) {
			if (HUsers[hUser].name.toString().toLowerCase() === name.toLowerCase()) {
				...
				mod.toClient('S_SPAWN_USER', 17, HUsers[hUser]);
				delete HUsers[hUser];
				break;
			}
		}
	...
	function ShowSpecificNpcByHzTi(hz, ti) {
		for (const hNpc in HNpcs) {
			if (HNpcs[hNpc].huntingZoneId === hz && HNpcs[hNpc].templateId === ti) {
				mod.toClient('S_SPAWN_NPC', 12, HNpcs[hNpc]);
				delete HNpcs[hNpc];
				break;
			}
		}
	...
	function ShowSpecificNpcByGid(gameId) {
		mod.toClient('S_SPAWN_NPC', 12, HNpcs[gameId]);
		delete HNpcs[gameId];
	}
	...
	function UpdateUserLoc(event) {
		mod.toClient('S_USER_LOCATION', 6, { gameId: event.gameId, loc: event.loc, w: event.w, speed: 3E2, dest: event.loc, type: 7 });
	}
	...
	mod.hook('S_SPAWN_USER', 17, LastHook, sSpawnUser)
	mod.hook('S_SPAWN_USER', 17, LastHookfn, sSpawnUserfn)
	mod.hook('S_DESPAWN_USER', 3, sDespawnUser)
	mod.hook('S_USER_LOCATION', 6, LastHook, sUserLocation)
	mod.hook('S_USER_STATUS', 4, LastHook, sUserStatus)
	...
	mod.hook('S_PARTY_MEMBER_LIST', 9, sPartyMemberList)
	mod.hook('S_LEAVE_PARTY_MEMBER', 2, sLeavePartyMember)
	mod.hook('S_LEAVE_PARTY', 'raw', sLeaveParty)

	mod.hook('S_SPAWN_NPC', 12, LastHook, sSpawnNpc)
	...
	mod.hook('S_ABNORMALITY_BEGIN', 5, LastHookfn, sAbnormalityBegin)
	...
	mod.hook('S_SPAWN_PROJECTILE', 6, LastHook, sSpawnProjectile)
	...
	mod.hook('S_EACH_SKILL_RESULT', mod.majorPatchVersion < 110 ? 14 : 15, LastHook, sEachSkillResult)
	mod.hook('S_ITEM_EXPLOSION_RESULT', 2, LastHook, sItemExplosionResult)
	mod.hook('S_SPAWN_DROPITEM', 9, LastHook, sSpawnDropItem)
	...
	Cmd.add(['fps', '!fps', 'fps-utils', '!fps-utils'], (key, arg, arg2, arg3) => {
		...
		switch (key) {
			...
			case "m": case "mod": case "mode": case "state":
				switch (arg) {
					...
					case "3": case "three":
						...
						break;
					default:
						Msg(`<font color="${GryC}">Invalid arg: '${arg}'</font>.`);
			...
			case "hide":
				...
				} else Msg(`<font color="${GryC}">Invalid arg: '${arg2}'</font>.`);
				break;
			case "show":
				if (typeof arg === "string" && arg !== null) {
			...
			case "hit":
				switch (arg) {
					...
					case "all":
						...
						break;
					default:
						Msg(`<font color="${GryC}">Invalid hit arg: '${arg}'</font>.`);
						break;
				}
				break;
				...
				switch (arg) {
					...
					case "blacklist":
						if (!arg2 || !arg3) {
							...
							} else return Msg(`<font color="${GryC}">Invalid abnormalities Blacklist arg: '${arg}'</font>.`);
						}
						break;
					case "log":
					case "debug":
						...
						break;
					default:
						Msg(`<font color="${GryC}">Invalid abnormalities arg: '${arg}'</font>.`);
						break;
				}
				break;
			case "guildlogo":
				...
				switch (arg) {
					...
					case "blacklist":
						if (!arg2 || !arg3) {
							...
							} else return Msg(`<font color="${GryC}">Invalid projectile Blacklist arg: '${arg}'</font>.`);
						}
						break;
					case "log": case "debug":
						ProjDebug = !ProjDebug;
						...
						break;
					default:
						Msg(`<font color="${GryC}">Invalid projectile arg: '${arg}'</font>.`);
						break;
				}
				break;
			case "quicklink":
