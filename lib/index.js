...
module.exports = function FpsUtils(mod) {
	...
	function sSpawnDropItem(event) {
		...
		if (mod.settings.HideBlacklistedDrop) {
			if (DropBlacklist.includes(event.item)) return false;
		}
		if (mod.settings.ShowStyle) {
			event.explode = 0;
			return true;
	...
	mod.command.add(['fps', 'fps-utils', 'fpsutil', 'fpsutils'], (key, arg, arg2, arg3) => {
		if (notcaaliproxy) mod.saveSettings(); // for some reason settings for caali's proxy weren't saving so we have this here now I guess 🤷
		switch (key) {
			case "g":
			case "gui":
				GuiHandler(arg, arg2);
			...
			case "dropitem":
				...
				Msg(`Drop blacklist ${mod.settings.HideBlacklistedDrop ? 'Hidden'.clr(GreenClr) : 'Shown'.clr(RedClr)}.`);
				break
			case "help":
