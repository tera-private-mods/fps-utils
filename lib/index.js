...
module.exports = function FpsUtils(mod) {
	var notcaaliproxy = mod.proxyAuthor !== 'caali';

	const NpcData = require(`./data/npcData.json`)
	const SkillData = require(`./data/skillString.json`)
	const BronzeClr = '#BC8F8F', SilverClr = '#9A9A9A', GoldClr = '#DAA520', PinkClr = '#FFB7C5', HPinkClr = '#FF69B4', CPinkClr = '#ED5D92', RedClr = '#FE6F5E', GreenClr = '#4DE19C', LPurpleClr = '#E0B0FF', PurpleClr = '#9966CC', LBlueClr = '#4DDCE1', BlueClr = '#436EEE', DBlueClr = '#00575e', HBlueClr = '#08B3E5', GrayClr = '#778899', YellowClr = '#DCE14D', OrangeClr = '#FFBE5E';
	...
	if (notcaaliproxy) mod.settings = require('./setting_migration')(mod.settings._version, require('../module').options.settingsVersion, mod.settings)
	...
	const Xmap = new WeakMap();	//ok I know what you think...if you got a better way feel free to inform me, thank you.
	if (!Xmap.has(mod)) {
		Xmap.set(mod, {});
	...
	function X1(Xarray) {
		...
		for (let i = 0; i < Xarray.length; i++) {
			if (!Xdone) {
				if (Xarray[i].command) Xshove(`<a href='admincommand:/@${Xarray[i].command}'>${Xarray[i].text}</a>`)
				else if (!Xarray[i].command) Xshove(`${Xarray[i].text}`)
				else console.log(`--v9-ERROR in Fps-Utils, pls report this as this ain't supposed to happen.`)
			} else if (Xdone) break;
			else console.log(`---ERROR in Fps-Utils, pls report this as this ain't supposed to happen.`)
		}
		return Xdata
	}

	function Xshove(stuff) {
		...
		switch (page) {
			...
			case "npcMain":
				...
				TmpData.push(
					...
					{ text: `<font color="${mod.settings.HideBlacklistedNpcs ? GreenClr : RedClr}" size="+20"><p align="right">[Blacklisted NPCs are ${mod.settings.HideBlacklistedNpcs ? 'Hidden' : 'Shown'}]</p></font><br><br>`, command: `fps npc;fps gui npcMain` },
			...
			case "show":
				...
				TmpData.push(
					...
					{ text: `<font color="${RedClr}" size="+16">Red</font><font color="${LPurpleClr}" size="+16"> = Shown, <font color="${GreenClr}" size="+16">Green</font><font color="${LPurpleClr}" size="+16"> = Hidden</font></font><br>`, command: `fps gui` },
					...
					{ text: `<font color="${LBlueClr}" size="+19">Click on <font color="${RedClr}">Red</font> to hide & add to blacklist. | on <font color="${GreenClr}">Green</font> to show & remove from blacklist</font><br>` }
				)
				for (let i in SpawnedPlayers) {
					TmpData.push({ text: `<font color="${mod.settings.PlayersBlacklist.includes(SpawnedPlayers[i].name) ? GreenClr : RedClr}" size="+17">${SpawnedPlayers[i].name}</font>`, command: mod.settings.PlayersBlacklist.includes(SpawnedPlayers[i].name) ? `fps show ${SpawnedPlayers[i].name};fps gui show` : `fps hide  ${SpawnedPlayers[i].name};fps gui show` })
				}
			...
			case "skills":
				gui.parse([
					{ text: `<font color="${PurpleClr}" size="+24"><p align="right">MAIN MENU</p></font><br>`, command: `fps gui` },
					{ text: `<font color="${mod.settings.HideBlacklistedSkills ? GreenClr : RedClr}" size="+22"><p align="right">[Blacklisted skills are ${mod.settings.HideBlacklistedSkills ? 'Hidden' : 'Shown'}]</p></font><br><br>`, command: `fps skill blacklist;fps gui skills` },
			...
			case "proj":
				...
				TmpData = []
				break;
			case "help": //(Noto Sans JP-KR-SC-TC_NL) *hopefully* now it will be read-able by all regions.
				gui.parse([
					{ text: `<font color="${PurpleClr}" size="+24"><p align="right">MAIN MENU</p></font><br>`, command: `fps gui` },
					{ text: `<font size="20">` },
					{ text: `<font color='${HBlueClr}'>  Command(s)</font>                  <font color='${DBlueClr}'>Argument(s)</font>                      <font color='${HPinkClr}'>Example</font><br>` },
					...
					{ text: `<font color='${HBlueClr}'>  mode  ~  state</font>              <font color='${DBlueClr}'>0  ~  1  ~  2  ~  3</font>                 <font color='${HPinkClr}'>!fps mode 2</font><br>` },
					...
					{ text: `<font color='${HBlueClr}'>summons ~ sums</font>             <font color='${DBlueClr}'>other ~  me</font>                   <font color='${HPinkClr}'>!fps sums me</font><br>` },
					{ text: `<font color='${HBlueClr}'>    skill ~ skills</font>               <font color='${DBlueClr}'>blacklist ~ Class arg</font>     <font color='${HPinkClr}'>!fps skill blacklist</font><br>` },
					{ text: `<font color='${HBlueClr}'>   npc  ~ npcs</font>                    <font color='${DBlueClr}'>N/A ~  hide arg</font>                  <font color='${HPinkClr}'>!fps npc</font><br>` },
					...
					{ text: `<font color='${HBlueClr}'>abn ~ abnormal</font>          <font color='${DBlueClr}'>all~blacklist arg arg2</font>    <font color='${HPinkClr}'>!fps abn blacklist</font><br>` },
					{ text: `<font color='${HBlueClr}'>         proj</font>                      <font color='${DBlueClr}'>all~blacklist arg arg2</font>         <font color='${HPinkClr}'>!fps proj all</font><br>` },
					{ text: `<font color='${HBlueClr}'>     guildlogo</font>                              <font color='${DBlueClr}'>N/A</font>                          <font color='${HPinkClr}'>!fps guildlogo</font><br>` },
					{ text: `<font color='${HBlueClr}'>style ~ costume</font>                        <font color='${DBlueClr}'>N/A</font>                             <font color='${HPinkClr}'>!fps style</font><br>` },
					{ text: `<font color='${HBlueClr}'> gui npcsearch</font>                          <font color='${DBlueClr}'>arg</font>                     <font color='${HPinkClr}'>!fps gui npcsearch E</font><br>` },
					{ text: `<font color='${HBlueClr}'>     npczoom</font>                              <font color='${DBlueClr}'>N/A</font>                          <font color='${HPinkClr}'>!fps npczoom</font><br>` },
					{ text: `<font color='${HBlueClr}'>     dropitem</font>                     <font color='${DBlueClr}'>N/A ~  hide arg</font>              <font color='${HPinkClr}'>!fps dropitem</font><br>` },
					...
					{ text: `<font color='${HBlueClr}'>    screenabns</font>                  <font color='${DBlueClr}'>N/A ~  hide arg</font>             <font color='${HPinkClr}'>!fps screenabns</font><br>` },
					{ text: `</font>` }
			...
			default:
				gui.parse([
					...
					{ text: "&nbsp;&nbsp;" },
					{ text: `<font color="${mod.settings.HideBlacklistedDrop ? GreenClr : RedClr}" size="+17">[Drops]</font><br>`, command: `fps dropitem;fps gui` },
					{ text: `&#09;&#09;&#09;` },
					{ text: `<font color="${mod.settings.HideMonsterDeathAnimation ? GreenClr : RedClr}" size="+17">[Monsters Death Animation]</font>`, command: `fps monsterdeathani;fps gui` },
					{ text: "&nbsp;&nbsp;" },
					{ text: `<font color="${mod.settings.HideOwnBlacklistedAbns ? GreenClr : RedClr}" size="+17">[Screen Abns]</font><br><br>`, command: `fps screenabns;fps gui` },
					...
					{ text: "&nbsp;&nbsp;" },
					{ text: `<font color="${mod.settings.ActionScripts ? GreenClr : RedClr}" size="+17">[Npc zoom-ins]</font>`, command: `fps npczoom;fps gui` },
					{ text: `<br><br><br><br><br>` },
					...
					{ text: "&#09;&#09;&nbsp;&nbsp;&nbsp;" },
					{ text: `<font color="${CPinkClr}" size="+17">[!!Instant Exit!!]</font><br>`, command: `fps quicklink instantexit` }
	...
	function Msg(msg) {
		mod.command.message(notcaaliproxy ? `[FPS] ${msg}`.clr(LPurpleClr) : `${msg}`.clr(LPurpleClr))
	}
	...
	function ShowSpecificPlayer(name) {
		for (let i in HiddenUsers) {
			if (HiddenUsers[i].name.toString().toLowerCase() === name.toLowerCase()) {
				...
				mod.send('S_SPAWN_USER', 14, HiddenUsers[i]);
	...
	function ModifySkillResult(event) {
		...
		if (mod.settings.Hit_Damage) {
			event.damage = event.type = event.noctEffect = event.stackExplode = 0;
			modified = true;
		}
	...
	function sLogin(event) {
		...
		if (mod.settings.ProjDebug) {
			...
			Msg("Detected Projectile debug on, " + "turned off".clr(HPinkClr) + '.')
		}
		if (mod.settings.AbnDebug) {
			mod.settings.AbnDebug = false
			Msg("Detected Abnormalities debug on, " + "turned off".clr(HPinkClr) + '.')
		}
	}
	...
	function sLeaveParty() {
		...
		if (mod.settings.RaidAutoChange) {
			if (mod.settings.Mode !== 2 || !LastState) return;
	...
	function sPartyMemberList(event) {
		...
		if (mod.settings.RaidAutoChange) {
			if (event.members.length >= 18) {
				if (mod.settings.Mode === 3 || (mod.settings.Mode === 2 && LastState)) return;
	...
	function sUnicastTransformData(event) {
		if (EqGid(event.gameId) || event.gameId == 0) return;
		if (mod.settings.ShowStyle) return false;
		if (mod.settings.Hideguildlogos) {
			event.guildLogo = '';
			return true;
	...
	function sSpawnNpc(event) {
		...
		if (event.huntingZoneId === 1023) {
			if (mod.settings.HideFireworks && (event.templateId == 60016000 || event.templateId == 80037000)) return false;
	...
	function sEachSkillResult(event) {
		...
		if (!EqGid(event.target) && mod.settings.Hit_Other && (SpawnedPlayers[event.owner] || SpawnedPlayers[event.source])) {
			event.skill = { npc: 0, type: 0, id: 0 };
			event.id = event.damage = event.type = event.noctEffect = event.crit = event.stackExplode = event.blocked = event.reaction.skill = 0;
			return true;
		}
	}
	...
	//mod.hook('S_USER_CHANGE_FACE_CUSTOM', 2, { order: 9999 }, sUserChangeFaceCustom) opcode not public. :v
	...
	mod.command.add(['fps', '!fps', 'fps-utils', '!fps-utils'], (key, arg, arg2, arg3) => {
		switch (key) {
			...
			case "hide":
				...
				} else Msg(`Invalid ${arg2}.`.clr(GrayClr));
				break;
			case "show":
				if (typeof arg === "string" && arg !== null) {
					if (mod.settings.PlayersBlacklist.includes(arg)) {
					...
					} else if (!mod.settings.ClassesBlacklist.includes(arg) || !mod.settings.RolesBlacklist.includes(arg)) Msg(`Class/Role "${arg}"` + ` Already shown`.clr(RedClr) + '.');
					else if (!mod.settings.PlayersBlacklist.includes(arg)) Msg(`Player "${arg}"` + ` Already shown`.clr(RedClr) + '.');
					else Msg(`Invalid ${arg2}.`.clr(GrayClr));
				}
				break;
			case "party":
			...
			case "skill":
				switch (arg) {
					...
					case "class":
						...
						} else Msg(`Class ${arg2} not found.`.clr(RedClr));
						break;
				}
				break;
			case "npcs":
			...
			case "abnormalities":
				...
				break;
			case "guildlogo":
				if (ChangeOnCd) return Msg(`Try again in 3 seconds.`.clr(PinkClr));
				mod.settings.Hideguildlogos = !mod.settings.Hideguildlogos;
				mod.send('C_SET_VISIBLE_RANGE', 1, { range: 0 });
				setTimeout(() => mod.send('C_SET_VISIBLE_RANGE', 1, { range: LastVrange }), 1500);
				ChangeOnCd = true;
				setTimeout(() => ChangeOnCd = false, 2800);
			...
			case "quicklink":
				switch (arg) {
					case "parcel":
						...
						break;
					case "talents":
						mod.send('C_REQUEST_CONTRACT', 1, { type: 77 });
						break;
					case "broker":
			...
			case "npczoom":
				...
				Msg(`Npc zoom-ins ${mod.settings.ActionScripts ? "Disabled".clr(GreenClr) : "Shown".clr(RedClr)}.`);
				break;
			case "dropitem":
				if (arg == 'hide') {
					if (!arg2) {
			...
			case "monstersdeathani":
				...
				Msg(`Monsters Death Animation is ${mod.settings.HideMonsterDeathAnimation ? "Hidden".clr(GreenClr) : "Shown".clr(RedClr)}.`);
				break;
			case "screenabns":
				if (arg == 'hide') {
					if (!arg2) {
						mod.settings.HideOwnBlacklistedAbns = !mod.settings.HideOwnBlacklistedAbns;
						Msg(`Blacklisted ScreenAbns ${mod.settings.OwnAbnormalsBlacklist ? "Hidden".clr(GreenClr) : "Shown".clr(RedClr)}.`);
						break;
					}
					arg2 = Number(arg2);
					let found = mod.settings.OwnAbnormalsBlacklist.some(m => m === arg2);
					if (found) {
						Msg(`Abnormal id '${arg2}'` + " Removed from the blacklist".clr(RedClr) + '.')
						mod.settings.OwnAbnormalsBlacklist = mod.settings.OwnAbnormalsBlacklist.filter(obj => obj != arg2);
					} else {
						Msg(`Abnormal id '${arg2}'` + " Added to the blacklist".clr(GreenClr) + ".")
						mod.settings.OwnAbnormalsBlacklist.push(arg2)
					}
					return;
				} else mod.settings.HideOwnBlacklistedAbns = !mod.settings.HideOwnBlacklistedAbns;
				Msg(`Blacklisted ScreenAbns ${mod.settings.HideOwnBlacklistedAbns ? 'Hidden'.clr(GreenClr) : 'Shown'.clr(RedClr)}.`);
				break;
			case "help":
