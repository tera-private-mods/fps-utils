...
module.exports = function FpsUtils(mod) {
	...
	function GuiHandler(page, arg) {
		switch (page) {
			...
			case "help":
				gui.parse([
					{ text: mod.majorPatchVersion < 79 ? `<font color="${PnkC}" size="+15"><p align="right">The GUI is only visual in classic server, buttons doesn't work, thanks for understanding.</p></font><br>` : '' },
					{ text: `<font color="${PrpC}" size="+24"><p align="right">MAIN MENU</p></font><br>`, command: "fps gui" },
	...
	function ModifySkillResult(event) {
		...
		if (mod.settings.Hit_Damage && (EqGid(event.source) || EqGid(event.owner) || UserNPCs.includes(event.templateId) && event.skill.huntingZoneId === 1023)) {
			event.id = event.type = event.noctEffect = event.reaction.skill.id = 0;
	...
	//'S_USER_EXTERNAL_CHANGE' meh, I can't figure out this definition for classic sth is messed up dunno why. :\
